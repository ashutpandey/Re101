...
function scoreArticle(title: string, description: string) {
...
function isWithinTimeframe(dateStr: string, days: number = 7) {
...
async function searchSigmaRules(keywords: string[]) {
  ...
  try {
    ...
    console.error("Sigma search error:", e);
    return [];
  }
}

async function searchYARARules(keywords: string[]) {
  const query = keywords.slice(0, 2).join("+");
  if (!query) return null;
  try {
    ...
    console.error("YARA search error:", e);
    return [];
  }
}

async function fetchRuleContent(rawUrl: string) {
  try {
...
function IOCTag({ label, items }: { label: string; items: string[] }) {
...
function RuleModal({ article, onClose }: any) {
        ...
        <div style={{ flex: 1, overflow: "auto", padding: 20, display: "grid", gridTemplateColumns: selectedRule ? "1fr 1.4fr" : "1fr 1fr", gap: 16 }}>
          ...
          {selectedRule && (
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div style={{ fontSize: 11, color: (selectedRule as any).type === "sigma" ? "#00d4ff" : "#a78bfa", textTransform: "uppercase", letterSpacing: 1, marginBottom: 10, fontFamily: "Fira Code" }}>
                {(selectedRule as any).type === "sigma" ? "Σ" : "Y"} {(selectedRule as any).name}
              </div>
              <a href={(selectedRule as any).html_url} target="_blank" rel="noreferrer" style={{ fontSize: 10, color: "#475569", marginBottom: 10, wordBreak: "break-all" }}>{(selectedRule as any).html_url}</a>
...
function ArticleCard({ article, source }: any) {
...
export default function ThreatFeed() {
  ...
  const loadFeeds = useCallback(async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        const processed = data.map((article: any) => ({
          ...article,
          _source: FEEDS.find(f => f.name === article.source) || { name: article.source, color: '#94a3b8' },
          _score: scoreArticle(article.title, article.description),
          _keywords: extractKeywords(article.title + " " + article.description),
        }));
  ...
  const filtered = articles
    ...
    .filter(a => !search || a.title.toLowerCase().includes(search.toLowerCase()) || a._keywords.some((k: string) => k.includes(search.toLowerCase())))
    .sort((a, b) => sortBy === "score" ? b._score - a._score : new Date(b.pubDate).getTime() - new Date(a.pubDate).getTime());

  return (
    <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", minHeight: "100vh", background: "#0d1117" }}>
      ...
      <div style={{ padding: 20, overflowY: "auto" }}>
        <div style={{ display: "flex", gap: 10, marginBottom: 20, alignItems: "center", flexWrap: "wrap" }}>
          ...
          <button onClick={loadFeeds} disabled={loading} style={{ background: loading ? "rgba(255,255,255,0.02)" : "rgba(0,212,255,0.06)", border: "1px solid rgba(0,212,255,0.2)", color: "#00d4ff", borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: "Fira Code", opacity: loading ? 0.5 : 1 }}>
        ...
        {loading && articles.length === 0 ? (
          ...
          <div style={{ textAlign: "center", padding: 60, color: "#3d444d", fontFamily: "Fira Code" }}>No articles found</div>
        ) : (
